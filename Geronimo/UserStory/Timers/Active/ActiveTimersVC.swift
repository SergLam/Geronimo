...
final class ActiveTimersVC: UIViewController {
    ...
    func setTableLayout(){
        ...
        activeTimersTable.snp.makeConstraints{ make in
    ...
    func updateActiveTimers(){
        for (index, timer) in activeTimersTable.activeTimers.enumerated(){
            if !timer.check_timer_end(timer: timer) {
                ...
                activeTimersTable.deleteRows(at: [IndexPath(row: index, section: 0)], with: .automatic)
            }
        }
    ...
    func setActiveTimers(db_result: Results<TimerRealm>){
        ...
        for timer_realm in db_result{
            let timer = GeronimoTimer(timer_realm: timer_realm)
            if timer.lastNotificationID != nil, let time = timer.last_alarm_time{
                if time > Date(){
                    timer.failCount += 1
    ...
    @objc
    func incrementTimersNextAlarmTime(){
        
        for (index, timer) in activeTimersTable.activeTimers.enumerated() where  timer.isEnabled {
            
            switch timer.type{
            case TimerData.TimerType.up.rawValue:
                if timer.isNow || timer.beginDate < Date() {
                    timer.timeToNextAlarm += 1
                    let cell = activeTimersTable.cellForRow(at: IndexPath(row: index, section: 0)) as? ActiveTimerTableCell
    ...
    func updateLocalDBTimers(){
        for timer in activeTimersTable.activeTimers{
            let realm_timer = TimerRealm(timer: timer)
